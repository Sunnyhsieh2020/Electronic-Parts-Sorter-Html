...
def index():
    ...
    return render_template("index.html", menu=menu,items=rs, bookmark_list=session.get("bookmark_list", []), bookmark_names=bookmark_names)   # 所有arduino零件
...
def category():
    ...
    for item in items:
        if item["class_id"] == class_id:
            rs.append(item)
    bookmark_names = [x[0] for x in session.get("bookmark_list", [])]
    return render_template("category.html", menu=menu,items=rs, bookmark_list=session.get("bookmark_list", []), bookmark_names=bookmark_names)
